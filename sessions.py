...
class SessionInterface(FlaskSessionInterface):
    ...
    def get_sid(self, app, request ):
        ...
        #    logger.debug("cakker  %s" % pprint.pformat(caller))

        #logger.debug("app  %s" % pprint.pformat(app))
        #appo = _app_ctx_stack.top 
        #reqo =  _request_ctx_stack.top

        va = request.view_args
        
        #logger.debug("appo  %s" % pprint.pformat(appo))
        #logger.debug("reqo  %s" % pprint.pformat(reqo.view_args))
        #logger.debug("reqo  %s" % pprint.pformat(reqo.__storage__))
        #logger.debug("reqo  %s" % pprint.pformat(reqo.__dict__))
        
        #logger.debug("app2  %s" % pprint.pformat(app.url_map))
        #logger.debug("reqest  %s" % pprint.pformat(request.__dict__))
        #logger.debug("reqest  %s" % pprint.pformat(request.args))        
        sid = None # start with none
        if not sid:
        ...
        if not sid:
            if va:
                if 'sid' in va:
                    sid=va['sid']
                    logger.debug("request form view args  %s" % sid)
                else:
                    logger.debug("request view args  %s" % va)
            else:
                logger.debug("request has no view args")

        sid2 = request.cookies.get(app.session_cookie_name)
        logger.debug("cookie sid  %s" % sid)

        # if there is an override, we ignore the cookie.
        if sid :
            return sid
        else:
            return sid2
